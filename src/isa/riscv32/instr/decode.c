...
static def_DHelper(J) {
  sword_t mask = ~((s->isa.instr.r.imm20 << 20) - 1);
  sword_t simm = mask | (s->isa.instr.r.imm19_12 << 12) |
                 (s->isa.instr.r.imm11 << 11) | (s->isa.instr.r.imm10_1 << 1);
  //Log("decode format J, imm=0x%08x, mask=0x%08x", simm, mask);
...
static def_DHelper(B) {
  ...
  sword_t mask = ~((s->isa.instr.b.imm12 << 12) - 1);
  sword_t simm = mask | (s->isa.instr.b.imm11 << 11) | (s->isa.instr.b.imm10_5 << 5) | (s->isa.instr.b.imm4_1 << 1);
  decode_op_i(s, id_src2, simm, false);
...
def_THelper(main) {
  ...
  def_INSTR_IDTAB("??????? ????? ????? ??? ????? 11100 11", I     , ebreak);
